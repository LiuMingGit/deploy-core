...
public interface AppFileMapper {
     ...
     * @return
     */
    @Select("select id,appId, filename, path, optime,sign from base_app_file where appId=#{appId} and path=#{path}")
    List<FileDTO> loadAppFile(@Param("appId") int appId, @Param("path") String path);


    @Select("select id,appId, filename, path, optime,sign from base_app_file where appId=#{appId} and sign=#{sign}")
    List<FileDTO> loadAppFilesWithSign(@Param("appId") int appId, @Param("sign") int sign);

    @Select("select id,appId, filename, path, optime,sign from base_app_file where appId=#{appId} and path=#{path} and sign=#{sign}")
     ...
     * @return
     */
    @Insert({"insert into base_app_file(appId, filename, path, optime,sign) values(#{appId}, #{filename}, #{path}, #{optime, jdbcType=TIMESTAMP},#{sign})"})
     ...
     * @param id
     */
    @Delete({"delete from base_app_file where id=#{id}"})
